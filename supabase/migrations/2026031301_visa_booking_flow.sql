...
DO $$
BEGIN
  IF to_regtype('public.document_type') IS NULL THEN
  ...
  BEGIN
    ALTER TYPE public.document_type ADD VALUE 'travel_itinerary';
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END;

  BEGIN
    ALTER TYPE public.document_type ADD VALUE 'hotel_booking';
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END;
END $$;
